<script>
	jQuery(document).ready(function ($) {
		...
		$.event.add(window, "scroll", function () {
			if (is_fired === false) {
				var hT = $('<?= $args['scroll_target'] ?>').offset().top;
				hH = $('<?= $args['scroll_target'] ?>').outerHeight();
				wH = $(window).height();
				wS = $(this).scrollTop();
				if (wS > (hT + hH - wH)) {
					console.info('Ani_js:: scroll target <?= $args['scroll_target'] ?> reached, shutting down auto run');
					set_default();
					play_all();
